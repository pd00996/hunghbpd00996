...
function add_address($customer_id, $line1, $line2,
    ...
    global $db;
    $query = '
        INSERT INTO diachi (idkhachhang, diachi1, diachi2,
                               thanhpho, quanhuyen, mavung, sodienthoai)
...
function get_address($address_id) {
    global $db;
    $query = 'SELECT * FROM diachi WHERE iddiachi = :address_id';
    $statement = $db->prepare($query);
...
function disable_or_delete_address($address_id) {
    ...
    if (is_used_address_id($address_id)) {
        $query = 'UPDATE diachi SET ghichu = 1 WHERE iddiachi = :address_id';
        $statement = $db->prepare($query);
        $statement->bindValue(":address_id", $address_id);
        $statement->execute();
        $statement->closeCursor();
    } else {
        $query = 'DELETE FROM diachi WHERE iddiachi = :address_id';
        $statement = $db->prepare($query);
        $statement->bindValue(":address_id", $address_id);
        $statement->execute();
        $statement->closeCursor();
...
function is_used_address_id($address_id) {
    ...
    $query = "SELECT COUNT(*) FROM donhang WHERE iddiachithanhtoan = :value";
    $statement = $db->prepare($query);
    ...
    $query = "SELECT COUNT(*) FROM donhang WHERE iddiachigiaohang = :value";
    $statement = $db->prepare($query);
    $statement->bindValue(':value', $address_id);
    $statement->execute();
    $result = $statement->fetch();
